...
describe('Tests e2e', () => {
  ...
  describe('UserResolver', () => {
    ...
    describe('[Mutation] addUser', () => {
      ...
      it(`[09] Devrait retourner une erreur de validation si la date de naissance est définie dans le futur`, () => {
    ...
    describe('[Mutation] addEmail', () => {
      ...
      it(`Ne devrait pas ajouter un email à l'utilisateur s\'il est inactif`, () => {
        return request(app.getHttpServer())
          .post('/graphql')
          .send({
            query: `mutation {addEmail(userId: "${inactiveUserId}", address: "test@test.fr"){id address}}`,
          })
          .expect(200)
          .expect((res) => {
            console.log(res);
